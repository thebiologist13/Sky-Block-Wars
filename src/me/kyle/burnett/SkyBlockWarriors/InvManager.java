...
public class InvManager {
	...
	public void saveInv(Player p){
		...
		Main.Inv.set(p.getName() +  ".Main", InventoryUtil.getInstance().toBase64(main));
		Main.Inv.set(p.getName() + ".Armor", InventoryUtil.getInstance().toBase64(armor));
	...
	public void restoreInv(Player p){
		...
		Inventory inv = InventoryUtil.getInstance().fromBase64(Main.Inv.getString(p.getName() + ".Main"));
		Inventory armor = InventoryUtil.getInstance().fromBase64(Main.Inv.getString(p.getName() + ".Armor"));
